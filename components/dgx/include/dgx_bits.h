...
/**
 * @brief Pack a left-justified MSB-first bit run into a linear 1-bpp byte array.
 *
 * Writes @p len bits (1..8) from the top of @p msb_value into @p array starting
 * at bit position @p idx. Bits outside the run are preserved (read-modify-write).
 *
 * Bit layout: pixel/bit at index @p idx lives in @p array[idx/8] at mask
 * (0x80 >> (idx & 7)). @p msb_value must be left-justified — the bits to write
 * occupy its top @p len bits; lower bits are ignored.
 *
 * Preconditions: 1 <= len <= 8. Caller must ensure idx + len does not exceed
 * the array's bit capacity.
 *
 * Used by 1-bpp packed blits. For per-pixel writes prefer dgx_fill_buf_value_1.
 */
static inline void dgx_pack_bits_msb(uint8_t *array, size_t idx, uint8_t msb_value, uint8_t len)
{
    size_t ai  = idx >> 3;
    size_t bi  = idx & 7;
    ...
    if (ai == aei) {
        if (bi == 0 && len == 8) {
            ...
            uint8_t shifted = msb_value >> bi;
            uint8_t mask    = ((uint8_t)0xff << (8 - len - bi)) & ((uint8_t)0xff >> bi);
            array[ai]       = (array[ai] & ~mask) | (shifted & mask);
        }
    } else {
        /* head: bottom (8 - bi) bits of array[ai] */
        uint8_t evalue = msb_value >> bi;
        uint8_t mask   = (uint8_t)0xff << (8 - bi);
        array[ai]      = (array[ai] & mask) | (evalue & ~mask);
        /* tail: top (bei) bits of array[aei], where bei = (idx + len) & 7 */
        evalue     = (uint8_t)(msb_value << (8 - bi));
        size_t bei = (idx + len) & 7;
        mask       = (uint8_t)0xff >> bei;
        array[aei] = (array[aei] & mask) | (evalue & ~mask);
    }
}

/**
 * @brief OR-pack a left-justified MSB-first bit run into a linear 1-bpp byte
 *        array. Existing destination bits are preserved by OR (set-only blit).
 *
 * Same layout and preconditions as dgx_pack_bits_msb. Differs in that bits in
 * @p msb_value that are 0 do not clear the destination — useful for glyph
 * stamping where the source bitmap is a foreground mask.
 */
static inline void dgx_or_bits_msb(uint8_t *array, size_t idx, uint8_t msb_value, uint8_t len)
{
    size_t ai  = idx >> 3;
    size_t bi  = idx & 7;
    size_t aei = (idx + len - 1) >> 3;
    uint8_t mask_run; /* ones in the run, MSB-numbered within a byte */
    if (ai == aei) {
        mask_run  = (uint8_t)(((uint8_t)0xff << (8 - len - bi)) & ((uint8_t)0xff >> bi));
        array[ai] |= (uint8_t)(msb_value >> bi) & mask_run;
    } else {
        /* head */
        mask_run  = (uint8_t)((uint8_t)0xff >> bi);
        array[ai] |= (uint8_t)(msb_value >> bi) & mask_run;
        /* tail */
        size_t bei = (idx + len) & 7;
        mask_run   = (uint8_t)((uint8_t)0xff << (8 - bei));
        array[aei] |= (uint8_t)(msb_value << (8 - bi)) & mask_run;
    }
}

/**
 * @brief AND-NOT-pack: clear bits of a linear 1-bpp byte array where source
 *        mask bits are set. Useful for "erase glyph" passes.
 */
static inline void dgx_andnot_bits_msb(uint8_t *array, size_t idx, uint8_t msb_value, uint8_t len)
{
    size_t ai  = idx >> 3;
    size_t bi  = idx & 7;
    size_t aei = (idx + len - 1) >> 3;
    ...
    if (ai == aei) {
        mask_run  = (uint8_t)(((uint8_t)0xff << (8 - len - bi)) & ((uint8_t)0xff >> bi));
        array[ai] &= (uint8_t)~((uint8_t)(msb_value >> bi) & mask_run);
    } else {
        mask_run  = (uint8_t)((uint8_t)0xff >> bi);
        array[ai] &= (uint8_t)~((uint8_t)(msb_value >> bi) & mask_run);
        size_t bei = (idx + len) & 7;
        mask_run   = (uint8_t)((uint8_t)0xff << (8 - bei));
        array[aei] &= (uint8_t)~((uint8_t)(msb_value << (8 - bi)) & mask_run);
    }
}
...
static inline uint32_t dgx_read_buf_value(uint8_t color_bits, uint8_t **lp, int idx)
{
    ...
    case 4:
        return dgx_read_buf_value_4(lp, idx);
    case 8:
...
static inline uint8_t *dgx_fill_buf_value(uint8_t color_bits, uint8_t *lp, int idx, uint32_t value)
{
    ...
    case 4:
        return dgx_fill_buf_value_4(lp, idx, value);
    case 8:
        return dgx_fill_buf_value_8(lp, idx, value);
    case 12:
        return dgx_fill_buf_value_12(lp, idx, value);
    case 16:
        return dgx_fill_buf_value_16(lp, idx, value);
    case 18:
    case 24:
        return dgx_fill_buf_value_24(lp, idx, value);
    case 32:
        return dgx_fill_buf_value_32(lp, idx, value);
    default:
        return 0;
    }
}
...
#define DGX_FILL_BUFFER(color_bits, bpointer, idx, pic, color)              \
    do {                                                                    \
        switch (color_bits) {                                               \
        case 1:                                                             \
            for (int i = 0; i < pic; ++i)                                   \
                bpointer = dgx_fill_buf_value_1(bpointer, idx + i, color);  \
            break;                                                          \
        case 4:                                                             \
            for (int i = 0; i < pic; ++i)                                   \
                bpointer = dgx_fill_buf_value_4(bpointer, idx + i, color);  \
            break;                                                          \
        case 8:                                                             \
            for (int i = 0; i < pic; ++i)                                   \
        ...
        case 12:                                                            \
            for (int i = 0; i < pic; ++i)                                   \
                bpointer = dgx_fill_buf_value_12(bpointer, idx + i, color); \
            break;                                                          \
        case 16:                                                            \
            for (int i = 0; i < pic; ++i)                                   \
