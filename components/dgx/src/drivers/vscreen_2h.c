...
static void dgx_vscreen_2h_update_area(dgx_screen_t *_scr, int left, int right, int top, int bottom) {
    ...
    if (scr->left_screen->screen_subtype == DgxPhysicalScreenWithBus && scr->right_screen->screen_subtype == DgxPhysicalScreenWithBus &&
        ...
        size_t draw_buffer_len = DGX_MIN(lbus->buffer_len, rbus->buffer_len) / 2;
        top = DGX_MAX(0, top);
        ...
        uint16_t *lbuf = (uint16_t *)lbus->buffer;
        uint16_t *rbuf = (uint16_t *)rbus->buffer;
        for (int br = top; br <= bottom; ++br) {
            ...
            for (int bc = 0; bc < lw; ++bc) {
                *lbuf++ = *lp_src++;
                *rbuf++ = *rp_src++;
                uint32_t filled_size = lbuf - (uint16_t *)lbus->buffer;
                if (filled_size >= draw_buffer_len - 1) {
                    scr->left_screen->write_area(scr->left_screen, lbus->buffer, 16u * filled_size);
                    scr->right_screen->write_area(scr->right_screen, rbus->buffer, 16u * filled_size);
                    scr->left_screen->wait_buffer(scr->left_screen);
                    scr->right_screen->wait_buffer(scr->right_screen);
                    lbuf = (uint16_t *)lbus->buffer;
                    rbuf = (uint16_t *)rbus->buffer;
                }
            }
        }
        if (lbuf != (uint16_t *)lbus->buffer) {
            scr->left_screen->write_area(scr->left_screen, lbus->buffer, 16u * (lbuf - (uint16_t *)lbus->buffer));
            scr->right_screen->write_area(scr->right_screen, rbus->buffer, 16u * (lbuf - (uint16_t *)lbus->buffer));
        }
    } else {
        if (lw > 0) {
